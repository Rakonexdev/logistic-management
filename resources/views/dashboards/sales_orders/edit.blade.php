...
@section('content')
    ...
    <div class="glass form-panel">
        <form action="{{ route('sales-orders.update', $order->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    <input type="text" name="so_number" class="form-input" required value="{{ old('so_number', $order->so_number) }}">
                </div>
                
                <div class="form-group">
                    <label class="form-label">Customer Name *</label>
                    <input type="text" name="customer_name" class="form-input" required value="{{ old('customer_name', $order->customer_name) }}" placeholder="Enter Customer Name">
                </div>

                <div class="form-group">
                    <label class="form-label">Designation</label>
                    <input type="text" name="designation" class="form-input" value="{{ old('designation', $order->designation) }}" placeholder="Enter Designation (e.g. Procurement Manager)">
                </div>
                
                <div class="form-group">
                    <label class="form-label">Order Date *</label>
                    <input type="date" name="order_date" class="form-input" required value="{{ old('order_date', $order->order_date->format('Y-m-d')) }}">
                </div>
            </div>
            ...
            <table class="items-table" id="itemsTable">
                ...
                <tbody id="itemsBody">
                    @foreach($order->items as $index => $item)
                        <tr>
                            <td>
                                <input type="text" name="items[{{ $index }}][sku_code]" class="form-input" required placeholder="Enter SKU" value="{{ old("items.{$index}.sku_code", $item->sku_code) }}" onchange="resetStockIndicator(this)">
                            </td>
                            <td>
            ...
            <div class="form-grid">
                <div class="form-group">
                    ...
                    @if($order->excel_file_path)
                        <span style="font-size: 0.8rem; color: var(--text-secondary);">Currently: <a href="{{ Storage::url($order->excel_file_path) }}" target="_blank">Download Excel/CSV</a></span>
                    @endif
                </div>
                ...
                <div class="form-group">
                    ...
                    @if($order->pdf_file_path)
                        <span style="font-size: 0.8rem; color: var(--text-secondary);">Currently: <a href="{{ Storage::url($order->pdf_file_path) }}" target="_blank">Download PDF</a></span>
                    @endif
                </div>
            </div>
            ...
            <div class="form-group" style="margin-top: 1.5rem;">
                ...
                <textarea name="remarks" class="form-input" rows="4">{{ old('remarks', $order->remarks) }}</textarea>
            </div>
    ...
    <script>
        ...
        // Run stock check automatically on load
