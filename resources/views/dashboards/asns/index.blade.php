...
@section('content')
    <div class="page-header">
        ...
        <a href="{{ route('asns.create') }}" class="btn btn-primary">
            <i class="ph ph-plus"></i> Create ASN
        </a>
    </div>
    ...
    <div class="glass">
        <form method="GET" action="{{ route('asns.index') }}" class="toolbar" id="filterForm">
            <div class="search-box">
                <i class="ph ph-magnifying-glass"></i>
                <input type="text" name="search" value="{{ request('search') }}" class="search-input" placeholder="Search by ASN Ref, AWB, Vendor..." oninput="debouncedSearch()">
        ...
        <div class="table-container">
            <table class="data-table">
                ...
                <tbody>
                    @forelse($asns as $asn)
                        <tr>
                            <td style="font-weight: 600;">{{ $asn->asn_reference }}</td>
                            <td>{{ $asn->vendor_id }}</td>
                            <td>{{ $asn->airway_bill }}</td>
                            <td>{{ $asn->items()->count() }} items</td>
                            <td>{{ $asn->created_at->format('M d, Y') }}</td>
                            <td>
        ...
        <div style="border-top: 1px solid var(--border-color);">
            {{ $asns->links('pagination::bootstrap-4') }}
        </div>
    </div>
@endsection
