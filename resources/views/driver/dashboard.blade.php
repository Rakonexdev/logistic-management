...
<head>
    ...
    <style>
        ...
        * {
        ...
        .badge-assigned { background: rgba(99, 102, 241, 0.15); color: var(--accent-secondary); border: 1px solid rgba(99, 102, 241, 0.3); }
        .badge-arrived { background: rgba(59, 130, 246, 0.15); color: var(--info); border: 1px solid rgba(59, 130, 246, 0.3); }
        .badge-delivered { background: rgba(16, 185, 129, 0.15); color: var(--success); border: 1px solid rgba(16, 185, 129, 0.3); }
        .badge-issue { background: rgba(239, 68, 68, 0.15); color: var(--danger); border: 1px solid rgba(239, 68, 68, 0.3); }
        .badge-started { background: rgba(245, 158, 11, 0.15); color: var(--warning); border: 1px solid rgba(245, 158, 11, 0.3); }
        .badge-completed { background: rgba(16, 185, 129, 0.15); color: var(--success); border: 1px solid rgba(16, 185, 129, 0.3); }
        .badge-warehouse { background: rgba(139, 92, 246, 0.15); color: var(--accent-primary); border: 1px solid rgba(139, 92, 246, 0.3); }
        ...
        .drawer-section p, .drawer-section div {
            font-size: 0.85rem;
            color: var(--text-primary);
        }
...
<div class="mobile-frame">
    ...
    <div class="app-content">
        ...
        <div id="panel-deliveries" class="tab-panel active">
            ...
            @else
                @foreach($deliveries as $order)
                    <div class="task-card" onclick="toggleDrawer(event, 'delivery-{{ $order->id }}')">
                        <div class="task-card-header">
                            <span class="task-ref">DEL-{{ $order->id }}</span>
                        ...
                        <!-- Expandable Drawer -->
                        <div id="drawer-delivery-{{ $order->id }}" class="task-drawer">
                            ...
                            @if($order->delivery_completed_at)
                                ...
                                @if($order->delivery_photo_path)
                                    <div class="drawer-section">
                                        ...
                                        <img src="{{ asset($order->delivery_photo_path) }}" class="image-preview" alt="Delivery Photo">
                                    </div>
                                @endif
                                @if($order->signed_proof_path)
                                    <div class="drawer-section">
                                        <h4>Signed Proof</h4>
                                        <img src="{{ asset($order->signed_proof_path) }}" class="image-preview" alt="Signature Proof">
                                    </div>
                                @endif
                            @endif
                            ...
                            @if($order->delivery_status === 'Assigned')
                                <form action="{{ route('driver.deliveries.arrive', $order->id) }}" method="POST">
                                    @csrf
                                    <button type="submit" class="btn btn-primary">
                                        <i class="ph ph-navigation-arrow"></i> Mark Arrived
                                    </button>
                                </form>
                                <button type="button" class="btn btn-danger" onclick="toggleIssueBox(event, 'delivery-issue-{{ $order->id }}')">
                                    <i class="ph ph-warning-octagon"></i> Report Issue
                                </button>

                                <div id="delivery-issue-{{ $order->id }}" class="issue-box">
                                    <form action="{{ route('driver.deliveries.issue', $order->id) }}" method="POST">
                                        @csrf
                            ...
                            @elseif($order->delivery_status === 'Arrived')
                                <form action="{{ route('driver.deliveries.complete', $order->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group" style="margin-top: 1rem;">
                                        ...
                                        <div class="form-file">
                                            <label for="signed_proof-{{ $order->id }}">Upload Signed Proof (POD)</label>
                                            <input type="file" id="signed_proof-{{ $order->id }}" name="signed_proof" accept="image/*" required>
                                        </div>

                                        <div class="form-file">
                                            <label for="delivery_photo-{{ $order->id }}">Upload Delivery Photo</label>
                                            <input type="file" id="delivery_photo-{{ $order->id }}" name="delivery_photo" accept="image/*" required>
                                        </div>
                                    </div>
                                ...
                                <div id="delivery-issue-{{ $order->id }}" class="issue-box">
                                    <form action="{{ route('driver.deliveries.issue', $order->id) }}" method="POST">
                                        @csrf
                                        <input type="text" name="delivery_issue" placeholder="Describe the issue" class="form-input" required>
                                        <button type="submit" class="btn btn-danger">Submit Issue</button>
                                    </form>
                                </div>
                            @endif
        ...
        <div id="panel-returns" class="tab-panel">
            <div class="section-title">
                Return Pickups
                <span>{{ $returns->whereIn('status', ['Pending Pickup', 'Pickup Started', 'Completed'])->count() }} tasks</span>
            </div>
                ...
                @foreach($returns as $ret)
                    <div class="task-card" onclick="toggleDrawer(event, 'return-{{ $ret->id }}')">
                        <div class="task-card-header">
                            ...
                            <span class="badge badge-{{ strtolower(str_replace(' ', '-', $ret->status === 'Returned to Warehouse' ? 'warehouse' : ($ret->status === 'Pickup Started' ? 'started' : ($ret->status === 'Completed' ? 'completed' : 'assigned')))) }}">
                                {{ $ret->status }}
                            </span>
                        </div>
                        <div class="task-detail">
                            <i class="ph ph-map-pin"></i>
                            <span>{{ $ret->pickup_location }}</span>
                        </div>
                        <div class="task-detail">
                            <i class="ph ph-tag"></i>
                        ...
                        <div id="drawer-return-{{ $ret->id }}" class="task-drawer">
                            ...
                            @elseif($ret->status === 'Pickup Started')
                                <form action="{{ route('driver.returns.complete', $ret->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <label class="form-label" style="font-size: 0.75rem; color: var(--text-secondary);">Quantity Picked Up</label>
                                        <input type="number" name="quantity_picked_up" value="{{ $ret->quantity }}" class="form-input" required min="0">
                                        
                                        <input type="text" name="condition_data" placeholder="Condition / Classification data (e.g. Damaged box, fully sealed)" class="form-input" required>
                                        ...
                                        <div class="form-file">
                                            ...
                                            <input type="file" id="return_photo-{{ $ret->id }}" name="photo" accept="image/*" required>
                                        </div>
                                    </div>
                                    <button type="submit" class="btn btn-success">
        ...
        <div id="panel-cheques" class="tab-panel">
            <div class="section-title">
                Cheque Collections
                <span>{{ $cheques->whereIn('status', ['Pending Collection', 'Collected'])->count() }} collections</span>
            </div>
                ...
                @foreach($cheques as $chq)
                    <div class="task-card" onclick="toggleDrawer(event, 'cheque-{{ $chq->id }}')">
                        <div class="task-card-header">
                            ...
                            <span class="badge badge-{{ strtolower(str_replace(' ', '-', $chq->status === 'Pending Collection' ? 'assigned' : ($chq->status === 'Collected' ? 'started' : ($chq->status === 'Submitted' ? 'completed' : 'issue')))) }}">
                                {{ $chq->status }}
                            </span>
                        </div>
                        <div class="task-detail">
                        ...
                        <div class="task-detail">
                            <i class="ph ph-map-pin"></i>
                            <span>{{ $chq->collection_location }}</span>
                        </div>
                        ...
                        <div id="drawer-cheque-{{ $chq->id }}" class="task-drawer">
                            ...
                            @if($chq->status === 'Pending Collection')
                                <form action="{{ route('driver.cheques.collect', $chq->id) }}" method="POST" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group" style="margin-top: 1rem;">
                                        <input type="text" name="remarks" placeholder="Remarks (e.g. Collected from receptionist)" class="form-input">

                                        <div class="form-file">
                                            <label for="cheque_photo-{{ $chq->id }}">Upload Cheque Photo</label>
                                            <input type="file" id="cheque_photo-{{ $chq->id }}" name="photo" accept="image/*" required>
                                        </div>
                                    </div>
                                    <button type="submit" class="btn btn-success">
                                ...
                                <button type="button" class="btn btn-danger" onclick="toggleIssueBox(event, 'cheque-issue-{{ $chq->id }}')">
                                    <i class="ph ph-warning-octagon"></i> Report Issue
                                </button>
                                <div id="cheque-issue-{{ $chq->id }}" class="issue-box">
                                    <form action="{{ route('driver.cheques.issue', $chq->id) }}" method="POST">
                                        @csrf
                                        <input type="text" name="remarks" placeholder="Describe the issue (e.g. Customer not present)" class="form-input" required>
                                        <button type="submit" class="btn btn-danger">Submit Issue</button>
                                    </form>
                                </div>
